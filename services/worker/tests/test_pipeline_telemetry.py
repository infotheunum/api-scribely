...
def test_dispatch_records_openrouter_error_in_app_settings(clean_db):
    ...
    error.details = lambda: "all OpenRouter keys exhausted: insufficient credits"

    patcher = patch("worker_app.dispatch.pipeline.rewrite_stub")
    stub = MagicMock()
    stub.EnrichCluster.side_effect = error
    patcher.return_value = stub
