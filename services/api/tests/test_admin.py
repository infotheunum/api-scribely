...
def test_generation_hours_get_and_put(client, admin_user, clean_db):
    ...
    assert body["weekend_daily_limit"] == 50
    assert len(body["days"]) == 7
    assert body["days"][5]["start_hour"] == 9
    assert body["days"][5]["end_hour"] == 12
    ...
    updated = client.put(
        ...
        json={
            ...
            + [
                {"weekday": 5, "enabled": True, "start_hour": 9, "end_hour": 14},
                {"weekday": 6, "enabled": False, "start_hour": 9, "end_hour": 12},
            ],
        },
        headers=headers,
    )
    assert updated.status_code == 200
    assert updated.json()["days"][5]["end_hour"] == 14
    assert updated.json()["days"][6]["enabled"] is False
    assert updated.json()["weekend_daily_limit"] == 50
    ...
    assert legacy.json()["weekend_daily_limit"] == 40
