...
from rewrite_app.prompt.versions import PROMPT_V7_NOTES
from sqlalchemy import select


def seed(db) -> PromptVersion:
    # The Admin-selected prompt is the source of truth. A service restart must
    # never silently retire it and resurrect the factory v7 template.
    active = db.scalar(
    ...
    version = db.scalar(select(PromptVersion).where(PromptVersion.notes == PROMPT_V7_NOTES))
    if version is None:
        version = PromptVersion(
            template=SYSTEM_PROMPT,
            status=PromptVersionStatus.DRAFT,
            notes=PROMPT_V7_NOTES,
        )
        db.add(version)
        db.flush()
...
def main() -> None:
    ...
    try:
        version = seed(db)
        print(f"active: {version.id} ({PROMPT_V7_NOTES})")
    finally:
        db.close()
