...
дано, ничего не додумывай и не округляй цифры.

Верни строго один JSON-объект по схеме:
{
  "facts": [{"kind": "who|what|when|number|quote|essence", "text": "..."}],
  "press_release": <bool>,
  "regulated": <bool>,
  ...
  "political_core": <bool>,
  "promotional_or_partner": <bool>,
  "exclusion_evidence": "<короткая цитата или точное объяснение, если один из флагов true>"
}
Обязательно:
- Один факт kind=essence: одно предложение — ГЛАВНАЯ суть новости
  (кто / что произошло), без оценок и без новых деталей.
- Все числа/суммы/%/цены/даты из текстов — отдельные kind=number (или when
  для дат), значение ТОЧНО как в источнике (не «около», не пересчёт).
- who / what / quote — имена, роли, действия, дословные цитаты.
...
- political_core: главная суть материала — политика (выборы, партии, политик,
  межгосударственный конфликт или политическая борьба), а не рынок/технология.
  Упоминание регулятора или закона само по себе не делает материал политическим.
- promotional_or_partner: это реклама, партнерский/спонсорский материал либо
  маркетинговый материал компании. Обычный новостной пресс-релиз помечай только
  press_release, если в нем нет рекламы или партнерского размещения.
- exclusion_evidence: заполни только при political_core или promotional_or_partner;
  укажи краткое основание строго из источника.
