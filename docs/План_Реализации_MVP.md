...
**Реальный баг, пойманный на деплое на Railway.** Миграция
(`ced7cc66d125`) добавляла `draft.sensitive_hold`/`compliance_notes`
как `NOT NULL` без `server_default`. Локально `draft` всегда была
пустой на момент прогона миграции (тесты/ручная проверка её чистят) —
`alembic upgrade head` проходил молча. На Railway `draft` уже не
пустая — живой диспетчер Фазы 4 к этому моменту создал реальные строки
— `ALTER TABLE ... ADD COLUMN ... NOT NULL` упал на
`NotNullViolation`, вся миграция в одной транзакции откатилась,
healthcheck `api` не поднялся за отведённое окно, деплой помечен
`FAILED` (Postgres/остальные сервисы не пострадали — старая версия
`api` осталась обслуживать трафик). Исправлено: оба `add_column`
получили `server_default` (`false` и `'{}'` соответственно) —
бэкфиллит существующие строки в той же операции. Воспроизведено и
проверено локально нарочно: понижена миграция на шаг, вставлена
реальная строка `Draft` в схему без новых колонок, миграция
поднята заново — прошла, существующая строка получила
`sensitive_hold=false`/`compliance_notes={}`. Урок на будущее: любая
